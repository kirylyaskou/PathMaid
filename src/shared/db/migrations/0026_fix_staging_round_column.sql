...
-- current schema (round INTEGER). Existing rows are preserved by id/content; `round` is
-- set to NULL for all rows (SQLite DDL has no `IF COLUMN EXISTS` so we can't branch).
-- Any round values from fresh databases will be lost — acceptable because the feature
-- is still in the staging pool phase and was not exposed to users in a tagged release.
